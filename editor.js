...
(function( $ ){
	...
	var methods = {
		...
		imageWidget: function(){
			...
			handleFlickerUpload = function(evt) {
				var files = evt.target.files;				
				var file_data = evt.target.files[0];   
			    var form_data = new FormData();                  
			    form_data.append('file', file_data);
			    form_data.append('upload_preset', 'run57d4l');
			          

				$.ajax({
				  url: 'https://api.cloudinary.com/v1_1/suyatidev/image/upload',
				  type: 'POST',
				  processData: false, // important
				  contentType: false, // important
				  dataType : 'text',
				  data: form_data,
				  success: function(cloud_response){
			            var data = jQuery.parseJSON(cloud_response);			            
			            $("#imageFromLinkBar").addClass("active");
			...
			uploadImageBar.append(chooseFromLocal.clone().on('change', handleFileSelect));
			imageToCloud.append(chooseFromLocal.clone().on('change', handleFlickerUpload));
			...
			tabContent.append(uploadImageBar).append(imageFromLinkBar).append(imageToCloud);
